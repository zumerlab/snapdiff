...
* License: MIT
*/
function z(a,t,e){return a*.29889531+t*.58662247+e*.11448223}function U(a,t,e){return a*.59597799-t*.2741761-e*.32180189}function F(a,t,e){return a*.21147017-t*.52261711+e*.31114694}function Q(a,t,e,n,r){let s=a[e],l=a[e+1],u=a[e+2],c=a[e+3],i=t[n],m=t[n+1],p=t[n+2],g=t[n+3];if(c===g&&s===i&&l===m&&u===p)return 0;c<255&&(c/=255,s=C(s,c),l=C(l,c),u=C(u,c)),g<255&&(g/=255,i=C(i,g),m=C(m,g),p=C(p,g));let x=z(s,l,u),y=z(i,m,p),b=x-y;if(r)return b;let o=U(s,l,u)-U(i,m,p),d=F(s,l,u)-F(i,m,p),h=.5053*b*b+.299*o*o+.1957*d*d;return x>y?-h:h}function C(a,t){return 255+(a-255)*t}function X(a,t,e,n,r,s){let l=Math.max(t-1,0),u=Math.max(e-1,0),c=Math.min(t+1,n-1),i=Math.min(e+1,r-1),m=(e*n+t)*4,p=t===l||t===c||e===u||e===i?1:0,g=0,x=0,y=0,b=0,o=0,d=0;for(let h=l;h<=c;h++)for(let v=u;v<=i;v++){if(h===t&&v===e)continue;let w=Q(a,a,m,(v*n+h)*4,!0);if(w===0){if(++p>2)return!1}else w<g?(g=w,y=h,b=v):w>x&&(x=w,o=h,d=v)}return g===0||x===0?!1:L(a,y,b,n,r)&&L(s,y,b,n,r)||L(a,o,d,n,r)&&L(s,o,d,n,r)}function L(a,t,e,n,r){let s=Math.max(t-1,0),l=Math.max(e-1,0),u=Math.min(t+1,n-1),c=Math.min(e+1,r-1),i=(e*n+t)*4,m=t===s||t===u||e===l||e===c?1:0;for(let p=s;p<=u;p++)for(let g=l;g<=c;g++){if(p===t&&g===e)continue;let x=(g*n+p)*4;if(a[i]===a[x]&&a[i+1]===a[x+1]&&a[i+2]===a[x+2]&&a[i+3]===a[x+3]&&++m>2)return!0}return!1}function T(a,t,e,n,r){a[t]=e,a[t+1]=n,a[t+2]=r,a[t+3]=255}function H(a,t,e,n){let r=a[t],s=a[t+1],l=a[t+2],u=C(z(r,s,l),e*a[t+3]/255);T(n,t,u,u,u)}function W(a,t,e,n,r,s={}){if(a.length!==t.length)throw new Error("Image data must have the same dimensions");let l=s.threshold??.1,u=!!s.includeAA,c=s.alpha??.1,i=s.aaColor??[255,255,0],m=s.diffColor??[255,0,0],p=!!s.diffMask,g=35215*l*l,x=n*r,y=0;if(a.length===t.length){let b=!0;for(let o=0;o<a.length;o++)if(a[o]!==t[o]){b=!1;break}if(b){if(e&&!p)for(let o=0;o<x*4;o+=4)H(a,o,c,e);return{diff:0,total:x,ratio:0}}}for(let b=0;b<r;b++)for(let o=0;o<n;o++){let d=(b*n+o)*4,h=Q(a,t,d,d,!1);Math.abs(h)>g?!u&&(X(a,o,b,n,r,t)||X(t,o,b,n,r,a))?e&&!p&&T(e,d,i[0],i[1],i[2]):(e&&T(e,d,m[0],m[1],m[2]),y++):e&&!p&&H(a,d,c,e)}return{diff:y,total:x,ratio:y/x}}function q(a,t,e={}){let n=Math.max(a.width,t.width),r=Math.max(a.height,t.height),s=a.width===t.width&&a.height===t.height,l=Y(a,n,r),u=Y(t,n,r),c=document.createElement("canvas");c.width=n,c.height=r;let i=c.getContext("2d"),m=i.createImageData(n,r),p=W(l,u,m.data,n,r,e);return i.putImageData(m,0,0),{...p,width:n,height:r,dimsMatch:s,canvas:c}}function Y(a,t,e){if(a.width===t&&a.height===e)return a.getContext("2d").getImageData(0,0,t,e).data;let n=document.createElement("canvas");n.width=t,n.height=e;let r=n.getContext("2d");return r.drawImage(a,0,0),r.getImageData(0,0,t,e).data}var et="snapDiff";var D="baselines";function A(){return new Promise((a,t)=>{let e=indexedDB.open(et,1);e.onupgradeneeded=()=>{let n=e.result;n.objectStoreNames.contains(D)||n.createObjectStore(D,{keyPath:"name"})},e.onsuccess=()=>a(e.result),e.onerror=()=>t(e.error)})}function R(a,t){return a.transaction(D,t).objectStore(D)}function M(a){return new Promise((t,e)=>{a.onsuccess=()=>t(a.result),a.onerror=()=>e(a.error)})}var S=class{constructor(t="default"){this.namespace=t}async _key(t){return`${this.namespace}::${t}`}async put(t,e,n={}){let r=await A(),s={name:await this._key(t),displayName:t,namespace:this.namespace,blob:e,width:n.width,height:n.height,createdAt:Date.now(),metadata:n.metadata??{}};return await M(R(r,"readwrite").put(s)),r.close(),s}async get(t){let e=await A(),n=await M(R(e,"readonly").get(await this._key(t)));return e.close(),n??null}async delete(t){let e=await A();await M(R(e,"readwrite").delete(await this._key(t))),e.close()}async list(){let t=await A(),e=await M(R(t,"readonly").getAll());return t.close(),e.filter(n=>n.namespace===this.namespace)}async clear(){let t=await this.list();for(let e of t)await this.delete(e.displayName)}async export(){let t=await this.list(),e=await Promise.all(t.map(async n=>({name:n.displayName,width:n.width,height:n.height,createdAt:n.createdAt,metadata:n.metadata,data:await at(n.blob)})));return{namespace:this.namespace,items:e,exportedAt:Date.now()}}async import(t,{overwrite:e=!1}={}){if(!t?.items)throw new Error("Invalid bundle");let n=0,r=0;for(let s of t.items){if(!e&&await this.get(s.name)){r++;continue}let l=await nt(s.data);await this.put(s.name,l,{width:s.width,height:s.height,metadata:s.metadata}),n++}return{added:n,skipped:r}}};async function j(a,t="image/png",e){return new Promise((n,r)=>{a.toBlob(s=>s?n(s):r(new Error("toBlob failed")),t,e)})}async function N(a){let t=URL.createObjectURL(a);try{let e=new Image;e.decoding="sync",e.src=t,await e.decode();let n=document.createElement("canvas");return n.width=e.naturalWidth,n.height=e.naturalHeight,n.getContext("2d").drawImage(e,0,0),n}finally{URL.revokeObjectURL(t)}}function at(a){return new Promise((t,e)=>{let n=new FileReader;n.onload=()=>t(n.result),n.onerror=()=>e(n.error),n.readAsDataURL(a)})}async function nt(a){return(await fetch(a)).blob()}function I(a={}){let{snapdom:t,namespace:e="default",threshold:n=.1,failureRatio:r=0,includeAA:s=!1,snapdomOptions:l={}}=a;if(!t)throw new Error("createRunner requires { snapdom }");let u=a.store??new S(e),c=[];function i(o,d,h={}){if(c.find(v=>v.name===o))throw new Error(`Duplicate test name: ${o}`);c.push({name:o,fn:d,options:h})}async function m(o){let d=await o.fn();if(d instanceof HTMLCanvasElement)return d;if(d instanceof Element){let h={...l,...o.options.snapdom??{}};return await t(d,h).then(v=>v.toCanvas())}throw new Error(`Test "${o.name}" must return an Element or HTMLCanvasElement`)}async function p(o){let d=performance.now();try{let h=await m(o),v=await u.get(o.name);if(!v)return{name:o.name,status:"new",actual:h,duration:performance.now()-d};let w=await N(v.blob),_={threshold:o.options.threshold??n,includeAA:o.options.includeAA??s},k=q(w,h,_),Z=o.options.failureRatio??r,tt=!k.dimsMatch||k.ratio>Z;return{name:o.name,status:tt?"fail":"pass",diff:k.diff,ratio:k.ratio,dimsMatch:k.dimsMatch,baseline:w,actual:h,diffCanvas:k.canvas,duration:performance.now()-d}}catch(h){return{name:o.name,status:"error",error:h,duration:performance.now()-d}}}async function g({filter:o,onProgress:d}={}){let h=o?c.filter(w=>o(w.name)):c,v=[];for(let w=0;w<h.length;w++){let _=await p(h[w]);v.push(_),d&&d({index:w,total:h.length,result:_})}return v}async function x(o,d){if(d||(d=(await p(c.find(w=>w.name===o))).actual),!d)throw new Error(`No actual canvas to approve for "${o}"`);let h=await j(d);await u.put(o,h,{width:d.width,height:d.height})}async function y(o){let d=o.filter(h=>h.status==="new"||h.status==="fail");for(let h of d)await x(h.name,h.actual);return d.length}function b(o){return{total:o.length,pass:o.filter(d=>d.status==="pass").length,fail:o.filter(d=>d.status==="fail").length,new:o.filter(d=>d.status==="new").length,error:o.filter(d=>d.status==="error").length}}return{test:i,run:g,approve:x,approveAll:y,summary:b,store:u,tests:c}}var rt=`
...
`;function lt({title:a="snapDiff report",results:t=[],generatedAt:e=Date.now(),baseDir:n="."}={}){let r=ct(t),s=t.map(u=>pt(u,n)).join(`
...
<script>${dt}<\/script>
</body></html>`}function ct(a){return{pass:a.filter(t=>t.status==="pass").length,fail:a.filter(t=>t.status==="fail").length,new:a.filter(t=>t.status==="new").length,error:a.filter(t=>t.status==="error").length}}function B(a,t){return t?`<span class="pill ${a}">${t} ${a}</span>`:""}function pt(a,t){let e=a.ratio!=null?`${(a.ratio*100).toFixed(2)}% (${a.diff} px)`:"",n=a.paths?.baseline?P(a.paths.baseline,t):"",r=a.paths?.actual?P(a.paths.actual,t):"",s=a.paths?.diff?P(a.paths.diff,t):"",l=a.dimsMatch===!1?" \xB7 <strong>dimensions differ</strong>":"";return`
...
</section>`}function O(a,t){return`<figure><img src="${t}" alt="${a}" /><figcaption>${a}</figcaption></figure>`}function ut(a,t){return`<div class="slider">
    <img src="${a}" alt="baseline" />
    <div class="layer" style="width:50%"><img src="${t}" alt="actual" /></div>
    <div class="handle" style="left:50%"></div>
  </div>`}function P(a,t){let e=String(a).replace(/\\/g,"/"),n=String(t).replace(/\\/g,"/").replace(/\/$/,"");return n&&e.startsWith(n+"/")?e.slice(n.length+1):e}function E(a){return String(a??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function K(a){let t=I(a);a.tests({test:t.test});let e=a.report!==!1?new $(t):null;if(e&&e.mount(),a.autoRun!==!1)if(e)await e.runAndShow();else return await t.run();return{runner:t,reporter:e}}K.createRunner=I;K.Reporter=$;export{S as BaselineStore,$ as Reporter,N as blobToCanvas,j as canvasToBlob,I as createRunner,q as diffCanvas,W as diffPixels,lt as generateStaticReport,K as snapDiff};
