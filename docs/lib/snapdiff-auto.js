...
* License: MIT
*/
var snapDiffAuto=(()=>{var $=Object.defineProperty;var tt=Object.getOwnPropertyDescriptor;var et=Object.getOwnPropertyNames;var nt=Object.prototype.hasOwnProperty;var at=(a,t)=>{for(var e in t)$(a,e,{get:t[e],enumerable:!0})},st=(a,t,e,n)=>{if(t&&typeof t=="object"||typeof t=="function")for(let s of et(t))!nt.call(a,s)&&s!==e&&$(a,s,{get:()=>t[s],enumerable:!(n=tt(t,s))||n.enumerable});return a};var ot=a=>st($({},"__esModule",{value:!0}),a);var yt={};at(yt,{bootstrap:()=>W});var rt="snapDiff";var D="baselines";function S(){return new Promise((a,t)=>{let e=indexedDB.open(rt,1);e.onupgradeneeded=()=>{let n=e.result;n.objectStoreNames.contains(D)||n.createObjectStore(D,{keyPath:"name"})},e.onsuccess=()=>a(e.result),e.onerror=()=>t(e.error)})}function A(a,t){return a.transaction(D,t).objectStore(D)}function E(a){return new Promise((t,e)=>{a.onsuccess=()=>t(a.result),a.onerror=()=>e(a.error)})}var R=class{constructor(t="default"){this.namespace=t}async _key(t){return`${this.namespace}::${t}`}async put(t,e,n={}){let s=await S(),o={name:await this._key(t),displayName:t,namespace:this.namespace,blob:e,width:n.width,height:n.height,createdAt:Date.now(),metadata:n.metadata??{}};return await E(A(s,"readwrite").put(o)),s.close(),o}async get(t){let e=await S(),n=await E(A(e,"readonly").get(await this._key(t)));return e.close(),n??null}async delete(t){let e=await S();await E(A(e,"readwrite").delete(await this._key(t))),e.close()}async list(){let t=await S(),e=await E(A(t,"readonly").getAll());return t.close(),e.filter(n=>n.namespace===this.namespace)}async clear(){let t=await this.list();for(let e of t)await this.delete(e.displayName)}async export(){let t=await this.list(),e=await Promise.all(t.map(async n=>({name:n.displayName,width:n.width,height:n.height,createdAt:n.createdAt,metadata:n.metadata,data:await it(n.blob)})));return{namespace:this.namespace,items:e,exportedAt:Date.now()}}async import(t,{overwrite:e=!1}={}){if(!t?.items)throw new Error("Invalid bundle");let n=0,s=0;for(let o of t.items){if(!e&&await this.get(o.name)){s++;continue}let l=await dt(o.data);await this.put(o.name,l,{width:o.width,height:o.height,metadata:o.metadata}),n++}return{added:n,skipped:s}}};async function j(a,t="image/png",e){return new Promise((n,s)=>{a.toBlob(o=>o?n(o):s(new Error("toBlob failed")),t,e)})}async function O(a){let t=URL.createObjectURL(a);try{let e=new Image;e.decoding="sync",e.src=t,await e.decode();let n=document.createElement("canvas");return n.width=e.naturalWidth,n.height=e.naturalHeight,n.getContext("2d").drawImage(e,0,0),n}finally{URL.revokeObjectURL(t)}}function it(a){return new Promise((t,e)=>{let n=new FileReader;n.onload=()=>t(n.result),n.onerror=()=>e(n.error),n.readAsDataURL(a)})}async function dt(a){return(await fetch(a)).blob()}function I(a,t,e){return a*.29889531+t*.58662247+e*.11448223}function N(a,t,e){return a*.59597799-t*.2741761-e*.32180189}function U(a,t,e){return a*.21147017-t*.52261711+e*.31114694}function H(a,t,e,n,s){let o=a[e],l=a[e+1],f=a[e+2],u=a[e+3],d=t[n],h=t[n+1],c=t[n+2],x=t[n+3];if(u===x&&o===d&&l===h&&f===c)return 0;u<255&&(u/=255,o=_(o,u),l=_(l,u),f=_(f,u)),x<255&&(x/=255,d=_(d,x),h=_(h,x),c=_(c,x));let v=I(o,l,f),y=I(d,h,c),g=v-y;if(s)return g;let r=N(o,l,f)-N(d,h,c),i=U(o,l,f)-U(d,h,c),p=.5053*g*g+.299*r*r+.1957*i*i;return v>y?-p:p}function _(a,t){return 255+(a-255)*t}function z(a,t,e,n,s,o){let l=Math.max(t-1,0),f=Math.max(e-1,0),u=Math.min(t+1,n-1),d=Math.min(e+1,s-1),h=(e*n+t)*4,c=t===l||t===u||e===f||e===d?1:0,x=0,v=0,y=0,g=0,r=0,i=0;for(let p=l;p<=u;p++)for(let b=f;b<=d;b++){if(p===t&&b===e)continue;let w=H(a,a,h,(b*n+p)*4,!0);if(w===0){if(++c>2)return!1}else w<x?(x=w,y=p,g=b):w>v&&(v=w,r=p,i=b)}return x===0||v===0?!1:M(a,y,g,n,s)&&M(o,y,g,n,s)||M(a,r,i,n,s)&&M(o,r,i,n,s)}function M(a,t,e,n,s){let o=Math.max(t-1,0),l=Math.max(e-1,0),f=Math.min(t+1,n-1),u=Math.min(e+1,s-1),d=(e*n+t)*4,h=t===o||t===f||e===l||e===u?1:0;for(let c=o;c<=f;c++)for(let x=l;x<=u;x++){if(c===t&&x===e)continue;let v=(x*n+c)*4;if(a[d]===a[v]&&a[d+1]===a[v+1]&&a[d+2]===a[v+2]&&a[d+3]===a[v+3]&&++h>2)return!0}return!1}function B(a,t,e,n,s){a[t]=e,a[t+1]=n,a[t+2]=s,a[t+3]=255}function F(a,t,e,n){let s=a[t],o=a[t+1],l=a[t+2],f=_(I(s,o,l),e*a[t+3]/255);B(n,t,f,f,f)}function lt(a,t,e,n,s,o={}){if(a.length!==t.length)throw new Error("Image data must have the same dimensions");let l=o.threshold??.1,f=!!o.includeAA,u=o.alpha??.1,d=o.aaColor??[255,255,0],h=o.diffColor??[255,0,0],c=!!o.diffMask,x=35215*l*l,v=n*s,y=0;if(a.length===t.length){let g=!0;for(let r=0;r<a.length;r++)if(a[r]!==t[r]){g=!1;break}if(g){if(e&&!c)for(let r=0;r<v*4;r+=4)F(a,r,u,e);return{diff:0,total:v,ratio:0}}}for(let g=0;g<s;g++)for(let r=0;r<n;r++){let i=(g*n+r)*4,p=H(a,t,i,i,!1);Math.abs(p)>x?!f&&(z(a,r,g,n,s,t)||z(t,r,g,n,s,a))?e&&!c&&B(e,i,d[0],d[1],d[2]):(e&&B(e,i,h[0],h[1],h[2]),y++):e&&!c&&F(a,i,u,e)}return{diff:y,total:v,ratio:y/v}}function X(a,t,e={}){let n=Math.max(a.width,t.width),s=Math.max(a.height,t.height),o=a.width===t.width&&a.height===t.height,l=P(a,n,s),f=P(t,n,s),u=document.createElement("canvas");u.width=n,u.height=s;let d=u.getContext("2d"),h=d.createImageData(n,s),c=lt(l,f,h.data,n,s,e);return d.putImageData(h,0,0),{...c,width:n,height:s,dimsMatch:o,canvas:u}}function P(a,t,e){if(a.width===t&&a.height===e)return a.getContext("2d").getImageData(0,0,t,e).data;let n=document.createElement("canvas");n.width=t,n.height=e;let s=n.getContext("2d");return s.drawImage(a,0,0),s.getImageData(0,0,t,e).data}var ct={dpr:1,scale:1,embedFonts:!0,invalidate:!0};async function Y(a,t,e={}){let n={...e};for(let[o,l]of Object.entries(ct))n[o]??(n[o]=l);return await(await a(t,n)).toCanvas({canvas:null})}function q(a={}){let{snapdom:t,namespace:e="default",threshold:n=.1,failureRatio:s=0,includeAA:o=!1,snapdomOptions:l={}}=a;if(!t)throw new Error("createRunner requires { snapdom }");let f=a.store??new R(e),u=[];function d(r,i,p={}){if(u.find(b=>b.name===r))throw new Error(`Duplicate test name: ${r}`);u.push({name:r,fn:i,options:p})}async function h(r){let i=await r.fn();if(i instanceof HTMLCanvasElement)return i;if(i instanceof Element){let p={...l,...r.options.snapdom??{}};return await Y(t,i,p)}throw new Error(`Test "${r.name}" must return an Element or HTMLCanvasElement`)}async function c(r){let i=performance.now();try{let p=await h(r),b=await f.get(r.name);if(!b)return{name:r.name,status:"new",actual:p,duration:performance.now()-i};let w=await O(b.blob),C={threshold:r.options.threshold??n,includeAA:r.options.includeAA??o},k=X(w,p,C),K=r.options.failureRatio??s,Z=!k.dimsMatch||k.ratio>K;return{name:r.name,status:Z?"fail":"pass",diff:k.diff,ratio:k.ratio,dimsMatch:k.dimsMatch,baseline:w,actual:p,diffCanvas:k.canvas,duration:performance.now()-i}}catch(p){return{name:r.name,status:"error",error:p,duration:performance.now()-i}}}async function x({filter:r,onProgress:i}={}){let p=r?u.filter(w=>r(w.name)):u,b=[];for(let w=0;w<p.length;w++){let C=await c(p[w]);b.push(C),i&&i({index:w,total:p.length,result:C})}return b}async function v(r,i){if(i||(i=(await c(u.find(w=>w.name===r))).actual),!i)throw new Error(`No actual canvas to approve for "${r}"`);let p=await j(i);await f.put(r,p,{width:i.width,height:i.height})}async function y(r){let i=r.filter(p=>p.status==="new"||p.status==="fail");for(let p of i)await v(p.name,p.actual);return i.length}function g(r){return{total:r.length,pass:r.filter(i=>i.status==="pass").length,fail:r.filter(i=>i.status==="fail").length,new:r.filter(i=>i.status==="new").length,error:r.filter(i=>i.status==="error").length}}return{test:d,run:x,approve:v,approveAll:y,summary:g,store:f,tests:u}}var pt=`
...
`,G=!1;function mt(){if(G)return;let a=document.createElement("style");a.textContent=ht,document.head.appendChild(a),G=!0}function T(a,t){if(a==null||a==="")return t;let e=String(a).toLowerCase();return!(e==="false"||e==="0"||e==="no")}function vt(a){let t=a.dataset,e={};return t.namespace&&(e.namespace=t.namespace),t.selector&&(e.selector=t.selector),t.threshold!=null&&(e.threshold=parseFloat(t.threshold)),t.failureRatio!=null&&(e.failureRatio=parseFloat(t.failureRatio)),"includeAa"in t&&(e.includeAA=T(t.includeAa,!0)),t.snapdomUrl&&(e.snapdomUrl=t.snapdomUrl),"autoRun"in t&&(e.autoRun=T(t.autoRun,!0)),"autoShow"in t&&(e.autoShow=T(t.autoShow,!0)),e}async function wt(a){let t=await import(a);return t.snapdom??t.default?.snapdom??t.default}function gt(a,t){let e=new Set,n=document.querySelectorAll(t),s=0;for(let o of n){let l=o.getAttribute("data-snap")||o.id;if(l||(l=`${o.tagName.toLowerCase()}-${s}`),e.has(l)){console.warn(`[snapDiff] duplicate test name "${l}" \u2014 skipping element`,o);continue}e.add(l);let f=o;a.test(l,()=>f),s++}return s}async function W(a={}){let{namespace:t="snapdiff-auto",selector:e="[data-snap]",threshold:n=.1,failureRatio:s=0,includeAA:o=!1,snapdomUrl:l="https://esm.sh/@zumer/snapdom@2",snapdomOptions:f={},autoRun:u=!0,autoShow:d=!1}=a,h=a.snapdom;if(!h)try{h=await wt(l)}catch(w){return console.error(`[snapDiff] failed to load snapdom from ${l}:`,w),null}if(typeof h!="function")return console.error("[snapDiff] snapdom is not a function \u2014 check the module exports at",l),null;let c=q({snapdom:h,namespace:t,threshold:n,failureRatio:s,includeAA:o,snapdomOptions:f});if(gt(c,e)===0)return console.warn(`[snapDiff] no elements matched selector "${e}" \u2014 nothing to test`),{runner:c,fab:null,reporter:()=>null,refresh:async()=>{}};mt();let v=document.createElement("div");v.className="sd-fab";let y=document.createElement("span");y.className="sd-dot";let g=document.createElement("span");g.textContent="snapDiff",v.append(y,g),document.body.appendChild(v);let r=w=>{if(v.classList.remove("sd-running","sd-has-fail","sd-has-new"),!w){g.textContent="snapDiff";return}let C=c.summary(w);C.fail||C.error?(v.classList.add("sd-has-fail"),g.textContent=`snapDiff: ${C.fail+C.error} fail`):C.new?(v.classList.add("sd-has-new"),g.textContent=`snapDiff: ${C.new} recorded`):g.textContent=`snapDiff: ${C.pass}/${w.length} \u2713`},i=null,p=null,b=async()=>{i?.root||(i||(i=new L(c,{onClose:()=>{p=i.results,r(p),i.unmount()}})),i.mount(),p?i.setResults(p):(p=await i.runAndShow(),r(p)))};if(v.onclick=b,u){v.classList.add("sd-running"),g.textContent="snapDiff: running\u2026";try{let w=await c.run();for(let k of w)k.status==="new"&&await c.approve(k.name,k.actual);p=w,r(w),(w.some(k=>k.status==="fail"||k.status==="error")||d)&&await b()}catch(w){console.error("[snapDiff] auto-run failed:",w),v.classList.remove("sd-running"),g.textContent="snapDiff: error"}}return{runner:c,fab:v,reporter:()=>i,refresh:b}}function xt(){if(typeof document>"u")return null;if(document.currentScript?.dataset?.auto!=null)return document.currentScript;let a=document.querySelectorAll("script[data-auto]");return a[a.length-1]??null}function bt(){if(typeof window>"u"||window.__snapdiffAutoTriggered)return;let a=xt();if(!a)return;window.__snapdiffAutoTriggered=!0;let t=vt(a),e=()=>W(t);document.readyState==="loading"?document.addEventListener("DOMContentLoaded",e,{once:!0}):e()}bt();return ot(yt);})();
